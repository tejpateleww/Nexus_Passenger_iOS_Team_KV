...
internal class WWClock: UIView {
    ...
    internal override func draw(_ rect: CGRect) {
        ...
        if showingHour {
            ...
            let textAttr = [NSAttributedStringKey.font: fontHour, NSAttributedStringKey.foregroundColor: fontColorHour, NSAttributedStringKey.paragraphStyle: paragraph] as [NSObject : Any]
            let textAttrHighlight = [NSAttributedStringKey.font.rawValue: fontHourHighlight, NSAttributedStringKey.foregroundColor: fontColorHourHighlight, NSAttributedStringKey.paragraphStyle: paragraph] as [NSObject : Any]
            
            let templateSize = NSAttributedString(string: "12", attributes: textAttr as? [NSAttributedStringKey : Any]).size()
            let templateSizeHighlight = NSAttributedString(string: "12", attributes: textAttrHighlight as? [NSAttributedStringKey : Any]).size()
            ...
            for (index, element) in hours.enumerated() {
                ...
                if element == currentHour {
                    ...
                    // numbers
                    let hour = NSAttributedString(string: "\(element)", attributes: textAttrHighlight as! [NSAttributedStringKey : Any])
                    ctx?.saveGState()
                    ctx?.scaleBy(x: -1, y: 1)
                    ctx?.translateBy(x: radiusHighlight * cos(angle), y: -(radiusHighlight * sin(angle)))
                    ctx?.scaleBy(x: -1, y: 1)
                    ...
                    // numbers
                    let hour = NSAttributedString(string: "\(element)", attributes: textAttr as! [NSAttributedStringKey : Any])
                    ctx?.saveGState()
                    ctx?.scaleBy(x: -1, y: 1)
            ...
            let textAttr : [NSAttributedStringKey : Any] = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): fontMinute, NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): fontColorMinute, NSAttributedStringKey.paragraphStyle: paragraph]
            let textAttrHighlight : [NSAttributedStringKey : Any] = [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): fontMinuteHighlight, NSAttributedStringKey(rawValue: NSAttributedStringKey.foregroundColor.rawValue): fontColorMinuteHighlight, NSAttributedStringKey.paragraphStyle: paragraph]
