...
class HomeViewController: BaseViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, GMSAutocompleteViewControllerDelegate, FavouriteLocationDelegate, UIPickerViewDelegate, UIPickerViewDataSource, NVActivityIndicatorViewable, UIGestureRecognizerDelegate, FloatRatingViewDelegate, CompleterTripInfoDelegate, ARCarMovementDelegate, GMSMapViewDelegate, addCardFromHomeVCDelegate, SelectCardDelegate,delegateRateGiven,deleagateForBookTaxiLater, ConfirmedDriverDelegate
{
    ...
    var alertForTip = AskForTipViewController()
    //        UIAlertController()
    var timer:Timer!
    ...
    @IBOutlet var btnShareLocation: ThemeButton!
    ...
    func didRatingCompleted() {
        ...
        TripviewController.arrData = self.arrDataAfterCompletetionOfTrip as NSMutableArray
    ...
    override func loadView() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.OpenChatList(notification:)), name: NotificationforDriverArrived, object: nil)
    ...
    override func viewDidLoad() {
        ...
        setPaymentType()
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(named: "icon_Favourite"), style: .plain, target: self, action: #selector(self.btnFavouriteAction))
        
        self.viewParentFromToLocation.backgroundColor = ThemeNaviBlueColor
        ...
        viewActivity.color = ThemeNaviBlueColor
        viewHavePromocode.tintColor = ThemeNaviBlueColor
        viewHavePromocode.stateChangeAnimation = .fill
        viewHavePromocode.boxType = .square
        viewTripActions.isHidden = true
        self.btnShareLocation.isHidden = true
        self.btnShareLocation.isHidden = true
        
        
        //        webserviceOfCardList()
        self.AddCardPopup.isHidden = true
        
        
        viewForMainFavourite.isHidden = true
        
        viewForFavourite.layer.cornerRadius = 10
    ...
    func OpenDriverDetailConfirmation(notification: NSNotification){
        
        DispatchQueue.main.async
            {
                if let notificationMessage = notification.userInfo as? String
                {
                    let driverMsg = notificationMessage
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        self.setNavBarWithMenu(Title: "Home".localized, IsNeedRightButton: false)
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: UIImage(named: "icon_Favourite"), style: .plain, target: self, action: #selector(self.btnFavouriteAction))
        //        setupGoogleMap()
        ...
        viewSelectPaymentOptionParent.layer.cornerRadius = 10
    ...
    func setLocalization()
    {
        ...
        btnRequestNow.backgroundColor = ThemeNaviBlueColor
    ...
    func webserviceCallForBookingCar()
    {
        ...
        dictParams.setObject(AirportRequest, forKey: SubmitBookingRequest.kAirportRequest as NSCopying)
        dictParams.setObject(AirportID, forKey: SubmitBookingRequest.kAirportId as NSCopying)
        ...
        if txtHavePromocode.text?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines) != "" {
    ...
    @IBAction func dismissBookNowView(_ sender: Any) {
        UIView.transition(with: self.viewBookNow, duration: 0.4, options: .transitionCrossDissolve, animations: {() -> Void in
            self.viewBookNow.isHidden = true
        }) { _ in }
    ...
    @IBAction func btnRequestNow(_ sender: UIButton) {
        ...
        if  self.txtHavePromocode.text?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines) != "" {
            var dictData = [String : AnyObject]()
            dictData["PromoCode"] = self.txtHavePromocode.text?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines) as AnyObject
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if pickerView == pickerViewForNoOfPassenger {
            ...
            txtSelectPaymentOption.text = ((data["CardNum2"] as? String) == "cash") ? "Cash" : (data["CardNum2"] as? String)
    ...
    func webserviceOfCardList() {
        ...
        webserviceForCardList(SingletonClass.sharedInstance.strPassengerID as AnyObject) { (result, status) in
            ...
            if (status) {
                ...
                // this code committed to stop self.aryCardsListForBookNow with cards list
                ...
                self.aryCardsListForBookNow.removeAll()
        ...
        self.socket.on(SocketData.kReceiveGetEstimateFare, callback: { (data, ack) in
            ...
            if (((data as NSArray).firstObject as? NSDictionary) != nil) {
                ...
                if estimateData[0]["estimate_fare"] != nil
                {
                    ...
                    if let IndexforSelectedVehicle =  self.selectedIndexPath as? IndexPath {
                        let dictOnlineCarData = (self.arrNumberOfOnlineCars.object(at:IndexforSelectedVehicle.row) as! NSDictionary)
                        let available = dictOnlineCarData.object(forKey: "carCount") as! Int
                        let checkAvailabla = String(available)
                        
                        if checkAvailabla != "0" {
                            self.strModelId = dictOnlineCarData.object(forKey: "Id") as! String
                        }
                        else {
                            self.strModelId = "0"
                        }
        ...
        placesClient.currentPlace(callback: { (placeLikelihoodList, error) -> Void in
            ...
            if let placeLikelihoodList = placeLikelihoodList {
                ...
                if let place = place {
                    ...
                    self.txtCurrentLocation.text = "\(place.name as! String) \(place.formattedAddress as! String)"
                    self.strLocationType = self.currentLocationMarkerText
    ...
    func bookingRequest()
    {
        ...
        if (SingletonClass.sharedInstance.CardsVCHaveAryData.count == 0) && self.aryCardsListForBookNow.count == 1 {
            self.addCardOptions()
            //                UtilityClass.showAlert("", message: "There is no card, If you want to add card than choose payment options to add card.", vc: self)
    ...
    @IBOutlet weak var AddCardPopup: UIView!
    @IBOutlet weak var lblAddCardMessage: UILabel!
    ...
    func addCardOptions() {
        ...
        self.lblAddCardMessage.text = "Do you want to add card?"
        
        UIView.transition(with: self.AddCardPopup, duration: 0.4, options: .transitionCrossDissolve, animations: {() -> Void in
            self.AddCardPopup.isHidden = false
        }) { _ in }
    }
    ...
    @IBAction func btnYes(_ sender: Any) {
        UIView.transition(with: self.AddCardPopup, duration: 0.4, options: .transitionCrossDissolve, animations: {() -> Void in
            ...
            //        NextnavigationController.isNavigationBarHidden = false
            //            self.navigationController?.present(NextnavigationController, animated: true, completion: nil)
        }
    ...
    // Share Location
    var TripPassengerLocation = String()
 
    @IBAction func btnShareLocation(_ sender: Any) {
        let textShare = [self.TripPassengerLocation]
...
//        "If you cancel the trip then you will be partially charged. Are you sure you want to cancel the trip?".localized
        let alert = UIAlertController(title: nil, message: "Are you sure you want to cancel the trip?", preferredStyle: .alert)
    ...
    @IBAction func btnFavouriteHideAction(_ sender: Any) {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell
    {
        ...
        {
            cell.viewOfImage.layer.cornerRadius = 30.0
            ...
            if dictOnlineCarData["carCount"] as! Int != 0 {
                ...
                if self.aryEstimateFareData.count != 0 {
                    ...
                    if ((self.aryEstimateFareData.object(at: indexPath.row) as! NSDictionary).object(forKey: "duration") as? NSNull) != nil {
                        ...
                        if let EstimateFare = (self.aryEstimateFareData.object(at: indexPath.row) as! NSDictionary).object(forKey: "total") as? Double {
                            cell.lblPrices.text = "\(currencySign)\(String(format: "%.2f", EstimateFare))"
                        }
                    }
                }
            }
        ...
        //                    if let arrLocation = dictOnlineCarData["locations"] as? [[String:Any]] {
        //                        if let CarTypeID = arrLocation[0]["CarType"] as? String {
        //                            for EstimateObject in self.aryEstimateFareData {
        //                                if let CarID = (EstimateObject as! NSDictionary).object(forKey: "id") as? Int {
        //                                    if "\(CarID)" == CarTypeID {
        //                                        let minute = (EstimateObject as! NSDictionary).object(forKey: "duration") as! Int
    ...
    @objc func GotoFavouritePage() {
        ...
        //            self.storyboard?.instantiateViewController(withIdentifier: "MyRatingViewController") as! MyRatingViewController
    ...
    func socketMethods()
    {
        ...
        socket.on(clientEvent: .connect) {data, ack in
            ...
            self.socket.on(SocketData.kNearByDriverList, callback: { (data, ack) in
                ...
                if self.txtCurrentLocation.text!.count != 0 && self.txtDestinationLocation.text!.count != 0 && self.aryOfOnlineCarsIds.count != 0 {
                    print("postPickupAndDropLocationForEstimateFare call from nearByDriverList from :  \(#function)")
                    self.postPickupAndDropLocationForEstimateFare()
    ...
    func socketMethodForGiveTipToDriverBookLater()
    {
        self.socket.on(SocketData.kAskForTipsToPassengerForBookLater, callback: { (data, ack) in
            ...
            /*
             ...
             textField.keyboardType = .decimalPadalertForTip
    ...
    func methodAfterStartTrip(tripData: NSArray) {
        ...
        self.viewTripActions.isHidden = false
        self.btnShareLocation.isHidden = false
        self.ConstantViewCarListsHeight.constant = 0
        self.viewCarLists.isHidden = true
        // self.viewFromToSubmitButton.isHidden = true
        //        self.viewShareRideView.isHidden = true
        ...
        self.webserviceToGetShareLocationURL()
    ...
    func showDriverInfo(bookingInfo : NSDictionary, DriverInfo: NSDictionary, carInfo : NSDictionary,Feedback : NSArray) {
        ...
        //         - \(next.strCarClass) - \(next.strCarPlateNumber)
    ...
    func onDriverArrviedBookLaterAtPickupLocation(){
        self.socket.on(SocketData.kArrivedDriverBookLaterRequest, callback: { (data, ack) in
                print("kArrivedDriverBookLaterRequest is \(data)")
    ...
    func stopTimertoUpdateLocation() {
        if timer != nil {
            timer.invalidate()
            timer = nil
        }
    ...
    func viewController(_ viewController: GMSAutocompleteViewController, didAutocompleteWith place: GMSPlace) {
        ...
        if BoolCurrentLocation {
            ...
            txtCurrentLocation.text = "\(place.name as! String) \(place.formattedAddress as! String)"
            ...
            txtDestinationLocation.text = "\(place.name as! String) \(place.formattedAddress as! String)"
    ...
    func getAirportLatLong() {
        ...
        webserviceToGetAirportsDetail { (result, status) in
            ...
            if status {
                ...
                if let AirportList = Response["aiport_latlng"] as? [[String:Any]] {
                    if self.VerifyUserIsInAirPort(AirportList: AirportList) {
                        self.AirportRequest = 1
                       self.bookingRequest()
                    } else {
                        self.AirportRequest = 0
                        self.AirportID  = "0"
                        self.bookingRequest()
                    }
                }
    ...
    func VerifyUserIsInAirPort(AirportList:[[String:Any]]) -> Bool {
       
        for Airport in AirportList {
            if let AirPortLocations = Airport["Location"] as? [[String:Any]] {
                 let Path = GMSMutablePath()
                for Location in AirPortLocations {
                    let AirportLat = Location["lat"] as! Double
                    let AirportLong = Location["lng"] as! Double
                  Path.add(CLLocationCoordinate2D(latitude: AirportLat, longitude: AirportLong))
                }
                if  GMSGeometryContainsLocation(self.defaultLocation.coordinate, Path , true) {
                    if let AirportRequestID = Airport["Id"] as? String {
                        AirportID = AirportRequestID
                    }
                    return true
                }
            }
                    ...
                    DispatchQueue.main.async( execute: { () -> Void in
                        ...
                        do{
                            ...
                            if status == "OK" {
                                ...
                                routePolyline.strokeColor = ThemeNaviBlueColor
                        ...
                        do{
                            ...
                            if status == "OK" {
                                ...
                                DispatchQueue.main.async {
                                    ...
                                    self.demoPolylineOLD = self.demoPolyline
                                    self.demoPolylineOLD.strokeColor = ThemeNaviBlueColor
                                    self.demoPolylineOLD.strokeWidth = 3.0
                                    self.demoPolylineOLD.map = self.mapView
                                    self.demoPolyline.map = nil
                                    ...
                                    self.demoPolyline = GMSPolyline(path: path)
                                    self.demoPolyline.map = self.mapView
                                    self.demoPolyline.strokeColor = ThemeNaviBlueColor
                                    self.demoPolyline.strokeWidth = 3.0
                                    self.demoPolylineOLD.map = nil
                    ...
                    DispatchQueue.main.async( execute: { () -> Void in
                        ...
                        do{
                            ...
                            if status == "OK" {
                                ...
                                self.routePolyline = GMSPolyline(path: path)
                                self.routePolyline.map = self.mapView
                                self.routePolyline.strokeColor = ThemeNaviBlueColor
                                self.routePolyline.strokeWidth = 3.0
    ...
    func webserviceToGetShareLocationURL() {
        
         let param = self.strBookingType + "/" + SingletonClass.sharedInstance.bookingId
        
        webserviceForSharePassengerLocation(param as AnyObject) { (result, status) in
            
            if (status) {
                let Response = result as! [String:Any]
                if let locationString = Response["location"] as? String {
                        self.TripPassengerLocation = locationString
                }
            }
      }
            ...
            if (status) {
                ...
                if bookingType != "" {
                    ...
                    if bookingType == "BookNow" {
                        ...
                        if let locationString = self.dictCurrentBookingInfoData["location"] as? String {
                            self.TripPassengerLocation = locationString
                        }
...
extension HomeViewController:AskForTipDelegate {
    
    func DidClickYes(Tips: String, isTripBookNow: Bool) {
        self.strTipAmount = Tips
        if isTripBookNow == true {
            let myJSON = ["Running": 0,"PassengerId" : SingletonClass.sharedInstance.strPassengerID, "Amount": self.strTipAmount, "BookingId": SingletonClass.sharedInstance.bookingId] as [String : Any]
            self.socket.emit(SocketData.kReceiveTips , with: [myJSON])
            print("kReceiveTips yes tip is given: \(myJSON)")
        } else {
